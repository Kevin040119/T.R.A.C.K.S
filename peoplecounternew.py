            ...
            if 3000 < cv2.contourArea(contour):
                ...
                if prev_centroid is not None and prev_dif is not None:
                    if (centroid[0] < line_pos < prev_centroid[0]) and (prev_dif < 250):
                        ...
                        query = "UPDATE trains SET jumlah_seat = %s WHERE gerbong = 1 AND train_id = %s"
                        Capacity += 1
                        values = (Capacity, IDkereta)
...
root = tk.Tk()
# Atur ukuran jendela
root.title = "Option People Counter"
window_width = 400
window_height = 300
